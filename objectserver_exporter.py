...
OSEXPORTER_SELF_OSSCRAPETIMETOTAL_SEC = Gauge('osexporter_self_osscrapetimetotal_sec', 'Time needed to scrape all data from one objectserver', ['hostserver', 'restport'])
OSEXPORTER_SELF_OSSCRAPETIMEALERTSSTATUS_SEC = Gauge('osexporter_self_osscrapetimealertsstatus_sec', 'Time needed to scrape all alerts.status from one objectserver', ['hostserver', 'restport'])
OSEXPORTER_SELF_OSSCRAPETIMEALERTSSTATUSCOLUMNS_SEC = Gauge('osexporter_self_osscrapetimealertstatuscolumns_sec', 'Time needed to scrape all alerts.status Field Details from one objectserver', ['hostserver', 'restport'])
OSEXPORTER_SELF_OSSCRAPETIMEMASTERSTATSROW_SEC = Gauge('osexporter_self_osscrapetimemasterstatsrow_sec', 'Time needed to scrape latest master stats info', ['hostserver', 'restport'])
OSEXPORTER_SELF_OSSCRAPEHTMLRETURNCODE_NUM = Gauge('osexporter_self_osscrapehtmlreturncode_num', 'The HTML Returncode from the Objectserver Rest API', ['hostserver', 'restport'])
OSEXPORTER_SELF_OSSCRAPETIMEMASTERPROBESTATS_SEC = Gauge('osexporter_self_osscrapetimemasterprobestats_sec', 'Time needed to scrape master.probestats info', ['hostserver', 'restport'])

logger.info('Finished setting prometheus metric definitions')

def getOsData(osRest, osRestPort, osLoginUser, osLoginPW, osProbeSelfMon, osProfileDetails, conTimeout, osImpactSelfMonitoring, osWebGUISelfMonitoring):
    osName = ''
    osEventsCritical = 0
    osEventsMajor = 0
    osEventsMinor = 0
    ...
    logger.info('Start gathering alerts.status data from ' + osRest)
    startTime = time.time()
    try:
        ...
        logger.debug('HTML Return Code from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory' + ' is: ' + str(alertsStatusSeverity.status_code))
        OSEXPORTER_SELF_OSSCRAPEHTMLRETURNCODE_NUM.labels(osRest, osRestPort).set(alertsStatusSeverity.status_code)
    ...
    try:
        ...
        OSEXPORTER_SELF_OSSCRAPETIMEALERTSSTATUS_SEC.labels(osRest, osRestPort).set(alertsStatusTime)
        logger.debug('Finished gathering data from ' + osRest + ':' + osRestPort + '/objectserver/restapi/alerts/status in ' + str(alertsStatusTime) + ' seconds')
    except:
    ...
    try:
        ...
        logger.debug('HTML Return Code for alerts.status Schema from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory' + ' is: ' + str(alertsColumnsDetails.status_code))
    ...
    try:
        ...
        OSEXPORTER_SELF_OSSCRAPETIMEALERTSSTATUSCOLUMNS_SEC.labels(osRest, osRestPort).set(alertsColumnsTime)
        logger.debug('Finished gathering alerts.status Schema data from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory in ' + str(alertsStatusTime) + ' seconds')
    except:
    ...
    try:
        ...
        logger.debug('HTML Return Code for master.stats Schema from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory' + ' is: ' + str(masterStatsRow.status_code))
    ...
    try:
        ...
        OSEXPORTER_SELF_OSSCRAPETIMEMASTERSTATSROW_SEC.labels(osRest, osRestPort).set(masterStatsRowTime)
        logger.debug('Finished gathering master.stats data from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory in ' + str(alertsStatusTime) + ' seconds')
    except:
    ...
    if osProbeSelfMon == '1':
        ...
        try:
            ...
            logger.debug('HTML Return Code for master.probestats from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory' + ' is: ' + str(masterStatsRow.status_code))
        except:
        ...
        try:
            ...
            OSEXPORTER_SELF_OSSCRAPETIMEMASTERPROBESTATS_SEC.labels(osRest, osRestPort).set(masterStatsRowTime)
            logger.debug('Finished gathering master.probestats data from ' + osRest + ':' + osRestPort + '/objectserver/restapi/sql/factory in ' + str(alertsStatusTime) + ' seconds')
        except:
    ...
    OSEXPORTER_SELF_OSSCRAPETIMETOTAL_SEC.labels(osRest, osRestPort).set(totalTime)
    logger.info('Finished gathering data from ' + osRest + ':' + osRestPort + ' in ' + str(totalTime) + ' seconds')
...
def helloOther():
    return "<h1>Hello Other World!</h1>"

@app.route("/os/<osserver>/<osport>")
def show_os_profile(osserver,osport):

    # show the user profile for that user
    osuser = request.args.get('username')
    #if osuser is None:
    #    osuser = ''
    ospw = request.args.get('password')
    if ospw is None:
        ospw = ''
    osArgProfileDetails = request.args.get('profileDetails')
    #if osArgProfileDetails is None:
    #    osArgProfileDetails = 0
    osArgProbeSelfMon = request.args.get('probeMon')
    #if osArgProbeSelfMon is None:
    #    osArgProbeSelfMon = 0
    osArgImpactSelfMonitoring = request.args.get('ImpactMon')
    #if osArgImpactSelfMonitoring is None:
    #    osArgImpactSelfMonitoring = 0
    osArgWebGUISelfMonitoring = request.args.get('WebGUIMon')
    #if osArgWebGUISelfMonitoring is None:
    #    osArgWebGUISelfMonitoring = 0 
    logger.info('execution data: ' + escape(osserver) + ' : ' + escape(osport) + ' : ' + escape(osuser) + ' : ' + escape(ospw) + ' : ' + escape(osArgProbeSelfMon) + ' : ' + escape(osArgProfileDetails) + ' : 5 : ' + escape(osArgImpactSelfMonitoring) + ' : ' + escape(osArgWebGUISelfMonitoring))
    getOsData(escape(osserver), escape(osport), escape(osuser), escape(ospw), escape(osArgProbeSelfMon) , escape(osArgProfileDetails), 5, escape(osArgImpactSelfMonitoring) , escape(osArgWebGUISelfMonitoring))
